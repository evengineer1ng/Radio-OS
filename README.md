...
Radio OS uses environment variables to configure global settings and paths. You can easily configure these in the **Settings > Environment** panel of the Radio OS Shell.

### Key Environment Variables

| Variable | Purpose | Example |
|----------|---------|---------|
| `RADIO_OS_ROOT` | Project root directory | `/path/to/Radio-OS-1.03` |
| `RADIO_OS_PLUGINS` | Global plugins directory | `/path/to/plugins` |
| `RADIO_OS_VOICES` | Voice models directory | `/path/to/voices` |
| `CONTEXT_MODEL` | Default producer LLM model | `qwen3:8b` |
| `HOST_MODEL` | Default host LLM model | `llama3.1:8b` |
| `OLLAMA_ENDPOINT` | Ollama server URL | `http://localhost:11434` |
| `PIPER_BIN` | Piper TTS executable path | `/path/to/piper/piper` |
| `OPENAI_API_KEY` | OpenAI API key | `sk-...` |
| `ANTHROPIC_API_KEY` | Anthropic API key | `sk-ant-...` |
| `GOOGLE_API_KEY` | Google Gemini API key | `AIza...` |

### Station-Specific Variables (Auto-Set)

These are automatically set for each launched station:
...
1. **GUI Method** (Recommended): Open Radio OS Shell → Settings → Environment tab
2. **Manual Method**: Edit the global config at:
